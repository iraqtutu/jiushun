...
| 角色 | 描述 | 核心权限 |
| :--- | :--- | :--- |
| **系统管理员 (admin)** | 最高权限管理者 | 登录、**全量工单查询 (高级筛选)**、**审批账号申请** |
| **玖顺员工** | 厂家内部技术人员 | 登录、新建工单、查看自己提交的工单 |
...
- **数据范围**：**查看所有**提交的服务单。
- **查询能力 (高级筛选)**：
  - **时间范围**：支持自定义开始/结束日期 (默认最近一个月)。
  - **模糊搜索**：支持 客户名称、客户手机号、报单人姓名、产品型号。
- **记录限制**：单次最多返回 **100** 条记录。

**B. 普通用户模式 (我的服务单)**
- **包含角色**：玖顺员工、经销商人员、服务人员。
- **入口名称**：我的服务单
- **数据范围**：**仅查看自己**提交的服务单。
- **查询能力**：
  - **时间范围**：仅支持自定义开始/结束日期。
...
### 3.4 意见反馈
- **功能目标**：收集用户的问题和建议，便于改进系统。
- **提交反馈**：
  - **类型**：问题 / 建议。
  - **内容**：文本描述（必填）。
  - **图片**：支持上传图片辅助说明。
- **反馈管理**：
  - **普通用户**：查看自己的反馈记录及管理员回复。
  - **管理员**：查看所有反馈，进行回复处理。
